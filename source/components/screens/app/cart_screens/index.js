...
const CartScreen = ({navigation, route}) => {
  ...
  const getlist = () => {
    AsyncStorage.getItem('userExist').then(res => {
      try {
        ...
        var config1 = {
          method: 'post',
          url: 'http://Ictkart.com/api/address/list',
          headers: {
            'Content-Type': 'application/json',
          },
          data: data,
    ...
    const removeItem = list => {
      AsyncStorage.getItem('userExist').then(res => {
        try {
          ...
          var config = {
            method: 'post',
            url: 'http://Ictkart.com/api/user/remove/cart',
            headers: {
              'Content-Type': 'application/json',
            },
            data: data,
      ...
      <MaterialCommunityIcons name="plus-circle" size={24} color="black" />
          ...
          <View
            ...
            <Text
              ...
              {list.currency}{' '}
              <Text style={{fontWeight: 'bold'}}>{list.sellingPrice}</Text>
            </Text>
            <Text
              style={{
                color: 'grey',
              ...
              {list.currency} {list.originalPrice}
            </Text>
          </View>
        </View>
      </View>
  ...
  const addAddress = () => {
    AsyncStorage.getItem('userExist').then(res => {
      ...
      var config = {
        method: 'post',
        url: 'http://Ictkart.com/api/address/add',
        headers: {
          'Content-Type': 'application/json',
        },
        data: data,
              ...
              <View style={{flex: 1}}>
                <View
                  ...
                  <Text style={{color: '#121212'}}>
                    ...
                    {state.cart.length > 1 ? 'items' : 'item'} in the cart{' '}
                  </Text>
                </View>
                <View
                  style={{
                ...
                <TouchableOpacity
                  ...
                  <Text
                    ...
                    Proceed to buy ( {state.cart.length}{' '}
                    {state.cart.length > 1 ? 'items' : 'item'} )
                  </Text>
                </TouchableOpacity>
                <FlatList
                  data={state.cart}
          ...
          <>
            <View
              ...
              <View
                ...
                <Text
                  style={{fontSize: FontSize(width * 0.05), color: '#5A429B'}}>
                  Order Summary
                </Text>
              </View>
              <View
