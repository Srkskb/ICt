...
const AccountScreen = ({navigation}) => {
  ...
  const getUserData = async currentUserData => {
    ...
    var response = await axios.get(
      `http://Ictkart.com/api/user/detail?userId=${currentUserData}`,
    );
  ...
  const submitForm = async () => {
    ...
    try {
      axios
        .post('http://Ictkart.com/api/user/update', data)
        .then(response => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={{flex: 1, alignItems: 'center', marginTop: 40}}>
        <TouchableOpacity
          onPress={opentheGallery}
          style={{
            width: 30,
            height: 30,
            borderRadius: 30,
            backgroundColor: '#493d88',
            alignItems: 'center',
            justifyContent: 'center',
          }}>
          {selectedimages !== null ? (
      ...
      <TouchableOpacity style={styles.buttonContainer} onPress={submitForm}>
